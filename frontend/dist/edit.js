import { api, Auth, renderNav, showFieldError, clearFieldError, clearAllFieldErrors, showAlert, hideAlert, setButtonLoading, inputVal, getParam, } from "./app.js";
renderNav();
if (!Auth.isLoggedIn()) {
...
if (backLink)
    backLink.href = `detail.html?id=${destId}`;
async function loadDestination() {
    try {
        const d = await api.get(`/destinations/${destId}`);
...
function prefillForm(d) {
    ...
    if (subtitle)
        subtitle.textContent = `Editing: ${d.title}`;
    document.getElementById("loading-state").style.display = "none";
...
async function submitForm() {
    ...
    try {
        await api.put(`/destinations/${destId}`, payload);
