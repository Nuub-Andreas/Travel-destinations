import { api, Auth, Modal, renderNav, escapeHtml, truncate, formatDateRange, setButtonLoading, } from "./app.js";
let allDestinations = [];
...
function renderCards(destinations) {
    ...
    container.innerHTML = `<div class="card-grid">${destinations.map((d, i) => buildCard(d, i, isLoggedIn, currentUser?.user_id ?? null)).join("")}</div>`;
...
function buildCard(d, i, isLoggedIn, currentUserId) {
...
function filterCards() {
    ...
    const filtered = allDestinations.filter(d => [d.title, d.country, d.location, d.description]
